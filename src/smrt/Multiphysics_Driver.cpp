    ...
    : d_assembly(assembly)
{
    ...
    Validate(nemesis::soft_equiv(z_edges.back(), d_assembly->height()),
             "End of axial edges " << z_edges.back() << " does not match "
             " assembly height " << d_assembly->height());
    ...
    {
        d_neutronics = std::make_shared<Two_Group_Diffusion>(
            assembly, neutronics_params, dz);
    }
    else
    {
#ifdef USE_SHIFT
        ...
        d_neutronics = std::make_shared<Shift_Solver>(
            assembly, shift_input, z_edges);
#else
        Validate(false, "Neutronics type set to 'shift', but Shift is not "
...
void Multiphysics_Driver::solve()
{
    ...
    for (int it = 0; it < d_max_iters; ++it)
    {
        ...
        if (power_diff / d_damping  < d_tol)
        {
            if (nemesis::node() == 0)
            {
                std::cout << "Coupled physics solve converged after "

...
//---------------------------------------------------------------------------//
auto Two_Group_Cross_Sections::get_data(Assembly_Model::PIN_TYPE type,
                                        double T,
                                        double rho) -> XS_Data
{
  Require(T > 273);
  ...
  if (type == Assembly_Model::FUEL) {
    ...
    auto interp = [T_base, rho_base, T, rho](const double coeffs[2]) {
      return coeffs[0] + coeffs[1] * (T - T_base) + coeffs[2] * (rho - rho_base);
    };
    ...
    auto interp = [rho_base, rho](const double coeffs[2]) {
      return coeffs[0] + coeffs[1] * (rho - rho_base);
    };

...
// Constructor
//---------------------------------------------------------------------------//
Single_Pin_Conduction::Single_Pin_Conduction(
        RCP_PL&                     parameters,
...
//---------------------------------------------------------------------------//
void Single_Pin_Conduction::solve(
        ...
        std::vector<double>&       fuel_temp)
{
    ...
    int num_rings_clad =
        std::ceil((d_clad_radius-d_fuel_radius) / d_delta_r_clad);
    ...
    std::fill(k.begin(),
              k.begin()+num_rings_fuel,
              d_k_fuel);
    std::fill(k.begin()+num_rings_fuel,
              k.end(),
              d_k_clad);
    ...
    // Check areas
    double fuel_area = std::accumulate(area.begin(),
                                       area.begin() + num_rings_fuel,
                                       0.0);
    ...
    for (int l = 0; l < num_levels; ++l)
    {
        ...
        for (int r = 1; r < num_rings-1; ++r)
        {
            ...
            matrix(r,r)   =
                2 * pi * k[r] * (radius[r] / dr_lo + radius[r+1] / dr_hi);
        ...
        rhs[num_rings-1] +=
            2 * pi * k.back() * channel_temp[l] * d_clad_radius / dr_hi;
